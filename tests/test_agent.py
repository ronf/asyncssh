...
class _TestAPI(AsyncTestCase):
    ...
    def asyncSetUpClass(cls):
        ...
        for alg_name in ('ssh-dss', 'ssh-rsa'):
            ckey = asyncssh.generate_private_key(alg_name)
            ...
            cls._public_keys[ckey.algorithm] = ckey.convert_to_public()
    ...
    def check_signature(self, key):
        """Check a signature generated by the agent"""

        sig = yield from key.sign(b'test')
        pubkey = self._public_keys[key.algorithm]
    ...
    def test_sign(self):
        ...
        for key in (yield from agent.get_keys()):
            yield from self.check_signature(key)
    ...
    def test_reconnect(self):
        ...
        yield from self.check_signature(keys[0])
        agent.close()
